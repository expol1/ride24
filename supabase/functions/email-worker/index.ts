...
.container{max-width:600px;margin:0 auto;background:#fff;border-radius:18px;overflow:hidden;box-shadow:0 10px 25px rgba(0,0,0,.05);border:1px solid #E2E8F0}
.header{padding:36px;text-align:center;border-bottom:1px solid #F1F5F9}
.content{padding:42px 34px;line-height:1.6}
.footer{padding:30px;text-align:center;font-size:13px;color:#64748B;background:#F8FAFC;border-top:1px solid #F1F5F9}
h1{font-size:26px;font-weight:800;color:#1E3A8A;margin-top:0;text-align:center}
p{margin-bottom:24px;font-size:16px;color:#334155}
.btn{display:inline-block;padding:15px 30px;background:#2563EB;color:#fff!important;text-decoration:none;border-radius:12px;font-weight:700;font-size:16px}
.highlight-box{background:#DCFCE7;border:1px solid #BBF7D0;padding:22px;border-radius:14px;margin-bottom:26px}
...
serve(async () => {
  ...
  for(const log of queue){
    try{
      // 🔥 WERSJA PRO: Pobieramy rezerwację OD RAZU z mailem klienta
      ...
      // 🔥 FIX: Najpierw bierzemy log.email (dla partnerów), a jeśli jest null, ratujemy się z profilu klienta!
      const finalEmail = log.email || booking.profiles?.email;

      // 🛡️ Twarde sprawdzenie typu (chroni przed błędem 422 z Resend)
      if (!finalEmail || typeof finalEmail !== "string") {
        console.error(`Pominięto wysyłkę dla log ID ${log.id} - brak lub błędny format adresu email.`);
        await supabase
          .from("email_logs")
      ...
      const res = await fetch("https://api.resend.com/emails",{
        ...
        body:JSON.stringify({
          ...
          to:finalEmail, // 🔥 Wstawiamy bezpieczny, pobrany email w formie stringa!
          subject,
          html
        })
      })
      ...
      if(res.ok){
        ...
        console.error(`Błąd Resend dla logu ${log.id}:`, errorData);
        await supabase
          .from("email_logs")
          .update({status:"failed"})
          .eq("id",log.id);
